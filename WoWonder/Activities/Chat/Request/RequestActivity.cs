...
using WoWonder.Activities.Chat.Request.Fragment;
...
namespace WoWonder.Activities.Chat.Request
{
        ...
        private async Task LoadGeneralData()
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    var (apiStatus, respond) = await RequestsAsync.Global.GetGeneralDataAsync(false, UserDetails.OnlineUsers, UserDetails.DeviceId, UserDetails.DeviceMsgId, "1", fetch);
                    if (apiStatus == 200)
                    {
                        if (respond is GetGeneralDataObject result)
                        {
                            RunOnUiThread(() =>
                            {
                                try
                                {
                                    ...
                                    {
                                        ListUtils.FriendRequestsList = FriendRequestTab.MAdapter.UserList = new ObservableCollection<UserDataObject>(result.FriendRequests);
                                    ...
                                    if (AppSettings.EnableChatGroup)
                                    {
                                        ...
                                        {
                                            ListUtils.GroupRequestsList = GroupRequestTab.MAdapter.GroupList = new ObservableCollection<GroupChatRequest>(result.GroupChatRequests);
