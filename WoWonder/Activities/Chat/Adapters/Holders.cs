...
namespace WoWonder.Activities.Chat.Adapters
{
    public class Holders
    {
        ...
        public class TextViewHolder : GlobalMessageViews
        {
            ...
            public TextViewHolder([NotNull] View itemView, Action<MesClickEventArgs> clickListener, Action<MesClickEventArgs> longClickListener, bool chatGroup) : base(itemView, chatGroup)
            {
                try
                {
                    ...
                    SuperTextView?.SetTextInfo(SuperTextView);
        ...
        public class CodeViewHolder : GlobalMessageViews
        {
            #region Variables Basic

            public View MainView { get; private set; }
            public HtmlTextView TextView { get; private set; }

            #endregion

            public CodeViewHolder([NotNull] View itemView, Action<MesClickEventArgs> clickListener, Action<MesClickEventArgs> longClickListener, bool chatGroup) : base(itemView, chatGroup)
            {
                try
                {
                    MainView = itemView;

                    TextView = itemView.FindViewById<HtmlTextView>(Resource.Id.active);
        ...
        public class SoundViewHolder : GlobalMessageViews, View.IOnClickListener
        {
            ...
            private void PlaySound(int position, MessageDataExtra message)
            {
                try
                {
                    ...
                    var mediaFile = ChatTools.GetFile(MessageAdapter.Id, Methods.Path.FolderDcimSound, fileName, message.Media, "audio");
                    ...
                    if (message.MediaPlayer == null)
                    {
                        ...
                        if (mediaFile.Contains("http"))
                            mediaFile = ChatTools.GetFile(MessageAdapter.Id, Methods.Path.FolderDcimSound, fileName, message.Media, "audio");
