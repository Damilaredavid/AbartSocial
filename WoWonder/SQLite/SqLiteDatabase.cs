    ...
    public class SqLiteDatabase
    {
        ...
        public ObservableCollection<AdapterModelsClassMessage> GetBroadcastMessages_List(string fromId, string broadcastId, string beforeMessageId)
        {
            try
            {
                ...
                var q = connection.Table<DataTables.BroadcastMessageTb>();
                var query = q.Where(w => w.BroadcastId == broadcastId).OrderBy(q => q.Time).ToList();
        ...
        public void Insert_Or_Update_one_LastUsersChat(ChatObject item)
        {
            try
            {
                ...
                if (chat != null)
                {
                    ...
                    chat.Okru = item.Okru;
                    chat.OpenToWorkData = item.OpenToWorkData;
        ...
        //Remove data from Users Table
        public void Delete_Archive(string id, string chatType, string recipientId = "")
        {
            try
            {
                using var connection = OpenConnection();
                ...
                if (chatType == "user")
                {
                    var user = connection.Table<DataTables.ArchiveTb>().FirstOrDefault(c => c.UserId == id && c.ChatType == "user");
                    if (user != null)
                        connection.Delete(user);
                }
                else if (chatType == "page")
                {
                    var page = connection.Table<DataTables.ArchiveTb>().FirstOrDefault(c => c.PageId == id && JsonConvert.DeserializeObject<MessageDataExtra>(c.LastMessage).ToData.UserId == recipientId && c.ChatType == "page");
                    if (page != null)
                        connection.Delete(page);
                }
                else if (chatType == "group")
                {
                    var group = connection.Table<DataTables.ArchiveTb>().FirstOrDefault(c => c.GroupId == id && c.ChatType == "group");
                    if (group != null)
                        connection.Delete(group);
                }
            }
            ...
            {
                if (e.Message.Contains("database is locked"))
                    Delete_Archive(id, chatType, recipientId);
                else
                    Methods.DisplayReportResultTrack(e);
            }
        }
