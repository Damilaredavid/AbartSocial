        ...
        public static void Initialize(Activity context)
        {
            try
            {
                if (AppSettings.ShowAppLovinBannerAds || AppSettings.ShowAppLovinInterstitialAds || AppSettings.ShowAppLovinRewardAds)
                {
                    ...
                    //settings.TestDeviceAdvertisingIds = new List<string>() { Methods.GetAdvertisingId(context) };
                    ...
                    var ad = AppLovinSdk.GetInstance(context.GetText(Resource.String.applovin_key), settings, context);
                    if (ad != null)
                    {
                        //ad.ShowMediationDebugger();  
                        ad.MediationProvider = AppLovinMediationProvider.Max;
                        ad.InitializeSdk(new MyAppLovinSdkInitialization(context));
                    }
