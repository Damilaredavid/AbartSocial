            ...
            try
            {
                ...
                switch (typePage)
                {
                    ...
                    case "BroadcastChatWindowActivity":
                        BroadcastActivityView = (BroadcastChatWindowActivity)activity;
                        break;
                }
            }
            catch (Exception e)
            ...
            try
            {
                ...
                switch (TypePage)
                {
                    ...
                    case "GroupChatWindowActivity":
                        ...
                        break;
                    case "BroadcastActivityView":
                        {
                            var data = JsonConvert.DeserializeObject<SendMessageObject>(json);
                            if (data?.Status == 200)
                            {
                                BroadcastMessageController.UpdateLastIdMessage(data);
                            }
                            else
                            {
                                var error = JsonConvert.DeserializeObject<ErrorObject>(json);
        ...
        public void UploadProgress(int uploadProgress)
        {
            try
            {
                ...
                switch (TypePage)
                {
                    ...
                    case "BroadcastChatWindowActivity":
                        {
                            var checker = BroadcastActivityView?.MAdapter?.DifferList?.FirstOrDefault(a => a.MesData?.Id == MessageModel.MessageHashId);
                            if (checker != null)
                            {
                                checker.MesData.MessageProgress = uploadProgress;
                                BroadcastActivityView?.RunOnUiThread(() => { BroadcastActivityView?.MAdapter.NotifyItemChanged(BroadcastActivityView.MAdapter.DifferList.IndexOf(checker), "WithoutBlobUploadProgress"); });
                            }
                        }
                        break;
                }
